 ...
 *
 *
 * @copyright  Copyright (c) 2007-2013 TOYOKOU-SOFTWARE INC. (http://www.toyokou-software.com)
...
class navigationControl extends SystemControl{
	...
	public function navigation_addOp(){
		...
		if (chksubmit()){
			...
			if ($error != ''){
				...
				switch ($_POST['nav_type']){
					 ...
					 */
					case '0':
						$insert_array['item_id'] = intval($_POST['diy_id']);
						break;
					/**
					 * 商品分类
					 */
		 ...
		 * 自定义分类
		 */
		$model_diy = Model('navigation_diy');
		$diy_list = $model_diy->select();
		/**
		 * 商品分类
		 */
		...
		if (chksubmit()){
			...
			if ($error != ''){
				...
				switch ($_POST['nav_type']){
					 ...
					 */
					case '0':
						$update_array['item_id'] = intval($_POST['diy_id']);
						break;
					/**
					 * 商品分类
					 */
	 ...
	 * 分类管理
	 */
	public function navigation_typeOp(){
		Tpl::showpage('navigation.type');
	}
	public function navigation_typeAjaxOp() {
		if (isset($_SERVER['REQUEST_METHOD']) && !empty($_SERVER['REQUEST_METHOD'])) {
			$model_navigation_type = Model('navigation_type');
			
			$request = $_SERVER['REQUEST_METHOD'];
			$input = explode("/", substr(@$_SERVER['PATH_INFO'], 1));
			switch ($request) {
				case 'POST':
					$data = array();
					parse_str(file_get_contents("php://input"),$data);
					if (!empty($data)){
						$model_navigation_type->insert($data);
					}
					// check if update successfully
					$data_retrieve = $model_navigation_type->order('type_id desc')->find();
					if ($data_retrieve['type_name'] == $data['type_name']) {
						$output['if_inserted'] = true;
					} else {
						$output['if_inserted'] = false;
					}
				...
				case 'PUT':
					...
					$model_navigation_type->where($condition)->update($data);
					
					// check if update successfully
					$data_retrieve['type_name'] = $model_navigation_type->getfby_type_id($data['type_id'], 'type_name');
					if ($data_retrieve['type_name'] == $data['type_name']) {
						$output['if_updated'] = true;
					} else {
				...
				case 'DELETE':
					...
					$output['input'] = $data;
					$condition['type_id'] = $data['type_id'];
					$model_navigation_type->where($condition)->delete();
					$data_retrieve['type_name'] = $model_navigation_type->getfby_type_id($data['type_id'], 'type_name');
					if (empty($data_retrieve['type_name'])) {
						$output['if_delete'] = true;
					} else {
						$output['if_delete'] = false;
					}
					echo json_encode($output);
					break;
				default:
	...
	public function ajaxOp(){
		switch ($_GET['branch']){
			...
			default:
				...
				$update_array['nav_id'] = intval($_GET['id']);
				$update_array[$_GET['column']] = trim($_GET['value']);
				$output['update'] = $update_array;
				$result = $model_navigation->update($update_array);
				
				// check update
				$condition[$_GET['column']] =  trim($_GET['value']);
				$sample = $model_navigation->where($condition)->find();
				if (!empty($sample)) {
