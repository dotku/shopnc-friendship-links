    ...
    <table class="table tb-type2">
      ...
      <tbody>
        ...
        <tr class="hover">
		  ...
		  <td><input onclick="linkEnable(this)" type="checkbox" <?php if ($v['if_approved'] == 1){?>checked="checked"<?php }?> data-id="<?php echo $v['nav_id']?>"></td>
...
<script>
	function linkEnable(obj){
		// window.console ? console.log($(obj).is(':checked')) : '';
		var status = $(obj).is(':checked');
		var nav_id = $(obj).data('id');
		// window.console ? console.log(nav_id) : '';
		if (status) {
			enable = 1;
		} else {
			enable = 0;
		...
		/**/
		$.ajax({
			url: "index.php?act=navigation&op=ajax&column=if_approved&value=" + enable + "&id=" + nav_id,
			type: 'GET',
			dataType : 'json',
			success: function(data){
				window.console ? console.log(data) : '';
				if (data && data.if_updated) {
					alert('更新成功');
				} else {
					alert('更新失败');
				}
